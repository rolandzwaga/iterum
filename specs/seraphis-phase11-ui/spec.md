...
| `struct UI::EditMessage` + message-ID constants | `src/ui/edit_message.h` (new) | The C-5 controller → processor wire format. **Eight kinds, 0–7** (kind 7 = `BlendBegin`, Q2). **AMENDED 2026-08-05 (Phase 12 hotfix, full-fidelity ruling): kind 8 = `PresetState`** — the preset browser's load path. The POD carries no data; a SECOND binary attribute (`kSeraphisStateAttributeId`, ≤ `kMaxPresetStateBytes`) holds a whole component-state stream, which `Processor::notify()` applies via its own `setState()` on the message thread (the project-load path — payloads, `[partials]` bits, mirror trackers, force-push all included). Coverage: `tests/integration/preset_load_test.cpp`. |
